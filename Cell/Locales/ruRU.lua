...
L["A priest tracks Fortitude and Spirit, a mage tracks Intellect"] = "На жреце останутся выносливость и дух, на маге - интеллект"
L["About"] = "Об аддоне"
...
L["Also available as |cFFFFB5C5/cell unlock|r - works in combat too"] = "Также работает |cFFFFB5C5/cell unlock|r - в том числе в бою"
L["Alpha"] = "Альфа"
L["Always"] = "Всегда"
...
--! WotLK feature: buff tracker "hide in combat", see UpdateHideInCombat.
L["Aura scanning stops as well, so this also saves CPU during a fight"] = "Проверка аур тоже останавливается, так что в бою это ещё и экономит процессор"
L["Author"] = "Автор"
...
L["Classes that provide none of these keep the whole list"] = "Класс, который ни одного из этих бафов не даёт, видит список целиком"
L["Clear"] = "Отчистить"
L["clear"] = "отчистить"
...
--! WotLK feature: raid debuffs can now be added by spell name, see F.SearchSpellsByName.
L["Create new debuff (id or name)"] = "Создать новый дебафф (id или название)"
L["Searching..."] = "Поиск..."
L["No spell found."] = "Заклинание не найдено."
...
--! WotLK fix: в переводе не было `%d`, а зовут строку через string.format
--! (Utilities/RaidTools.lua: "Текущее: " .. string.format(L["first %d"], n)).
--! Lua 5.1 на лишний аргумент НЕ падает - format молча возвращает строку без
--! числа, поэтому в подсказке «Отчёт о смертях» русский игрок видел «первый(я)»
--! без количества, и ни !BugGrabber, ни игровой прогон этого не показывали.
--! Восстановлен только сам плейсхолдер: слово переводчика оставлено, взята его
--! множественная форма, потому что «первый(я) 3» по-русски не читается.
L["first %d"] = "первые %d"
L["Flash"] = "Мерцание"
L["Focus"] = "Фокус"
L["focus"] = "Фокус"
...
L["Glow when you are missing the buff"] = "Подсветка, если баффа нет на тебе"
L["Glows"] = "Свечения"
L["Gradient"] = "Градиент"
...
L["move"] = "двигать"
L["Mover"] = "Двигалка"
L["Mover is off."] = "Двигалка выключена."
L["Mover is on: %s. Drag the green frame, then /cell lock."] = "Двигалка включена: %s. Тащи за зелёную рамку, потом /cell lock."
L["Mover is on, but no tool is enabled - turn one on in Raid Tools first."] = "Двигалка включена, но ни один инструмент не активен - сначала включи его в Рейдовых инструментах."
...
L["Only track buffs of your class"] = "Только бафы своего класса"
...
L["show or hide the movers of Marks Bar, Buff Tracker and ReadyCheck/PullTimer buttons"] = "показать или скрыть двигалки Панели Меток, Отслеживания Бафов и кнопок Проверки Готовности/Таймера Пула"
...
L["The icon stays either way"] = "Иконка остаётся в любом случае"
--! WotLK fix: настройки аггро-индикаторов, см. UnitButton_UpdateThreat.
L["hideForTanks"] = "Не показывать танкам"
L["A tank holds the mob by design"] = "Танк держит моба по должности - на 25ппл это половина мигающих рамок, которые ни о чём не сообщают"
--! WotLK feature: aggro blink pulse toggle, see I.SetAggroBlinkNoPulse.
L["noPulse"] = "Не мигать"
L["The highlight stays, only the blinking stops"] = "Подсветка аггро остаётся на месте, пропадает только мигание"
L["Threat threshold"] = "Порог угрозы"
--! WotLK feature: cluster counter, see Indicators/Cluster.lua.
L["Cluster"] = "Скопление"
L["Shows how many allies an area heal cast on this unit would reach."] = "Показывает, сколько союзников зацепит массовое лечение, если направить его на этого юнита."
L["Cluster radius"] = "Радиус скопления"
L["How far from a unit an ally still counts as standing with them"] = "На каком расстоянии от юнита союзник ещё считается стоящим вместе с ним"
L["0 means the radius of your own class and spec"] = "0 - взять радиус своего класса и таланта"
L["Paladin 9.5, Shaman 12.5, Holy Priest 16.5, Discipline Priest 30, Druid 16.5"] = "Паладин 9.5, шаман 12.5, жрец Свет 16.5, жрец Послушание 30, друид 16.5"
L["lowHealthOnly"] = "Считать только раненых"
L["Full-health allies are still standing there, they just do not need the heal"] = "Союзник с полным здоровьем стоит там же, просто лечение ему не нужно"
--! WotLK feature: incoming-heal counter, see Indicators/IncomingHealers.lua.
L["Incoming Healers"] = "Кто уже лечит"
L["Shows how many different healers already have a heal landing on this unit."] = "Показывает, сколько разных хилеров уже лечат этого юнита, а цветом - успеет ли ваше лечение раньше остальных."
L["countHoTs"] = "Считать HoT-ы"
L["A heal over time counts by its next tick, not by the moment it runs out"] = "Лечение со временем считается по следующему тику, а не по концу действия. Лайфблум не считается никогда - он всегда висит на танке"
L["My heal lands first"] = "Моё лечение придёт первым"
L["Another healer lands first"] = "Первым придёт чужое"
L["Percentage of the threat of whoever is holding the mob"] = "Процент от угрозы того, кто моба сейчас держит"
L["100 means the unit has caught up with them"] = "100 - юнит с ним сравнялся: это в точности прежнее поведение. Меньше - раннее предупреждение, больше - только реальный съём"
L["130 means only when the mob is already hitting the unit"] = "130 - только когда моба уже бьёт именно этот юнит: такого процента угрозы не бывает, поэтому верх ползунка означает сам факт съёма"
L["Without a hostile target Cell falls back to the old rule and ignores this"] = "Если враждебной цели нет вообще, Cell откатывается на старое правило и порог не действует"
...
L["Texture"] = "Текстура"
--! WotLK feature: buff tracker "hide in combat", see UpdateHideInCombat.
L["The bar and the Missing Buffs icons come back when the fight ends"] = "Полоса и иконки \"нет баффа\" вернутся, когда бой закончится"
L["The icon and the counter stay either way"] = "Иконка и счётчик остаются в любом случае"
