...
L["menu"] = "菜单"
L["clear"] = "清除"
--! WotLK fix: здесь стояло `L["set unit"] = "设置为目标单位"`, и эта строка не
--! работала НИКОГДА: тот же ключ присваивается второй раз ниже, в дописанном в
--! конец файла блоке Quick Assist (`L["set unit"] = "设置单位"`), а побеждает
--! последнее присваивание - молча, без ошибки. Убрана мёртвая строка, а не
--! перевод: на экране по-прежнему «设置单位», ни один символ не изменился.
--! Победившая строка к тому же верная - подсказка в RaidFrames/Groups/
--! SpotlightFrame.lua просит «Left-Drag: set unit», то есть «назначить юнита»,
--! а «设置为目标单位» значит «сделать целевым юнитом» (zhTW: 設定單位).
...
--! WotLK fix: тексты L["CHANGELOGS"] и L["OLDER_CHANGELOGS"] удалены по решению
--! владельца (GAP-087), причина - в Locales/enUS.lua. Тем же проходом убраны два
--! закомментированных остатка того же журнала (заметка про Afflicted Souls и древний
--! блок --[[ r25-release … r3-alpha ]]). Ключ L["Changelogs"] и обе подписи ссылок
--! выше оставлены: это однострочные переводы, вреда от них нет.
