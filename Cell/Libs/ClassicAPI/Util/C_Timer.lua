...
--! C_Timer is not native to 3.3.5a (checked against milkyway-codex), so the whole
--! namespace is built unconditionally and then merged key-by-key into _G.C_Timer:
--! a foreign C_Timer keeps its own After/NewTicker, and Cell can still reach OUR
--! implementation through CellClassicAPI.C_Timer.
local _, Private = ...

local Type = type
local SetMetaTable = setmetatable

-- Animation host. Reuse an existing TimerFrame (another addon or a foreign
-- ClassicAPI may have created it) instead of stomping the global.
local Host = TimerFrame or CreateFrame("Frame", "TimerFrame")

local C_Timer = {}
local Registry = SetMetaTable({}, { __mode = "k" }) -- maps the "Public" proxy table to the "Internal" Animation object
local Pool = {}
...
local function New()
	...
	local A = Host:CreateAnimationGroup()
	local Timer = A:CreateAnimation("Animation")
