...
--! WotLK feature: панель «Резервные копии» на вкладке «О Cell». Файл был выведен
--! из сборки при чистке, хотя вся его инфраструктура осталась живой:
--! `CellDBBackup` объявлена в Cell.toc как SavedVariable и инициализируется в
--! Core_Wrath.lua (`if type(CellDBBackup) ~= "table" then CellDBBackup = {} end`),
--! ключи локалей на месте во всех 11 файлах, иконки create/rename/delete2/backup
--! лежат в Media/Icons. То есть копии некуда было создавать только из-за
--! отсутствия UI. Никакой адаптации под 3.3.5 файл не требует: он рисуется
--! теми же хелперами Cell.*, что и соседний ImportExport.lua.
...
local function CreateItem(index)
    ...
    b.rename:SetScript("OnClick", function()
        local popup = Cell.CreatePopupEditBox(backupFrame, function(text)
            --! WotLK fix: было `date()` без формата - переименование без ввода
            --! давало дату в другом виде, чем создание. Формат один на оба места.
...
local function CreateBackupFrame()
    ...
    -- close
    local closeBtn = Cell.CreateButton(backupFrame, "X", "red", {18, 18}, false, false, "CELL_FONT_WIDGET_TITLE", "CELL_FONT_WIDGET_TITLE_DISABLE")
...
function LoadBackups()
    ...
    for i, t in pairs(CellDBBackup) do
        ...
        --! WotLK fix: `t["versionNum"] < Cell.MIN_VERSION` падало на копии без
        --! этого поля - "attempt to compare nil with number" рвало отрисовку
        --! всей вкладки «О Cell». Свои копии поле пишут всегда, но CellDBBackup
        --! - SavedVariable: там может лежать что угодно, включая таблицу,
        --! перенесённую игроком с другой сборки. Без поля копия считается
        --! непригодной для восстановления, а не роняет панель.
        if (t["versionNum"] or 0) < Cell.MIN_VERSION then
