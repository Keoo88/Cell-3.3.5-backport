...
local function UpdateSliderStatus()
    ...
    --! WotLK fix: unitsSlider was hidden unconditionally, but Combine Groups is alive
    --! here and unitsPerColumn IS applied - RaidFrame.lua:621 feeds it to the combined
    --! secure header, an attribute SecureGroupHeaderTemplate supports natively on 3.3.5.
    --! Without the slider the real grid stayed at 5 while the preview drew the DB value.
...
local function CreateLayoutSetupPane()
    ...
    combineGroupsCB = Cell.CreateCheckButton(pages.main, L["Combine Groups"].." ("..L["Raid"]..")", function(checked, self)
        ...
        F.IterateAllUnitButtons(function(b) b._indicatorsReady = nil end, true)
...
LoadLayoutDB = function(layout, dontShowPreview)
    ...
    if not selectedLayoutTable["main"]["roleOrder"] then
        selectedLayoutTable["main"]["roleOrder"] = {"TANK", "HEALER", "DAMAGER"}
    end
